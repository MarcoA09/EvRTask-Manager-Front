...
function RegisterPage() {
  const { signup, logout, errors: registerErrors, isAuthenticated, emailVerified, userverify } = useAuth();
    const { register, handleSubmit, setValue, formState: {errors}, setError } = useForm();
    const navigate = useNavigate();

     useEffect(() => {
      if(userverify) {
        console.log(userverify.rol)
    if (userverify.name) {
      setValue("name", userverify.name);
    }
    if (userverify.email) {
      setValue("email", userverify.email);
    }
    if (userverify.rol) {
      setValue("rol", userverify.rol);
    }
  }
     },[userverify, setValue]); 
...
<div className='flex h-[calc(100vh-100px)] items-center justify-center'>
    <div className='bg-zinc-800 max-w-md p-10 rounded-md'>
        ...
        <p>Por favor, completa los datos faltantes. Si el nombre tiene un error o está incompleto, corrige.</p>
