...
import { loginRequest, registerRequest, getUsersRequest, verifyTokenRequest, verifyEmailRequest, registerHomeRequest } from "../api/auth";
...
export const AuthProvider = ({ children }) => {
  ...
  const signuphome = async (user) => {
    try {
      const res = await registerHomeRequest(user);
      setUser(res.data);
      setIsAuthenticated(true);
      localStorage.setItem("username", res.data.username);
      localStorage.setItem("token", res.data.token);
      localStorage.setItem("rol", res.data.rol || "user");
      if (res.data.rol === "Colaborador") {
        localStorage.setItem("userRol", "Colaborador");
      }
